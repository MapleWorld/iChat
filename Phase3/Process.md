...
1. edit thread			|	|    |	 | |           |1 issue #67	|1 issue #67
2.		|		|   |		   |	|	|				|	2 issue #66
3.		|| |		   |	|			|           	|				3 issue #63
4. admin delete reply		|	|   |		   |	  |	|	|				4 issue #68
5.		|| |		   |	|	|				|			5 issue #64
6. admin create categories	||	   |	       ||	|6 issue #65	|			6 issue #65
7. reply to thread		|	|	 |	 |	|	|7 issue #60	|7 issue #60				
...
Using Kanban
----------------
We found it more difficult to adapt the Kanban strategy to our development process. We found that the Scrum strategy was more effective since Sprints were a simpler and more logical way to organize our work. Since client features often depended on server implementation first, we found it was an effective strategy to have a sprint focusing on server development, followed by a sprint for client development. With Kanban, it was harder to balance the workload of the team between the different tasks, and it was also more difficult to coordinate developing server and client features in the correct order. We tried to have a limit on how many client features were under development before the server-side implementation was ready, since it is easier to test the server without a client than it is to test the client without a server. Thus, we allowed a higher volume of server issues to be in progress at the beginning of our development cycle, and then shifted to focus on client issues later on. While this was similar to how we organized our sprints with Scrum, we found there was more overhead in managing the task allocation with Kanban. It would have required more time spent in meetings to decide how to organize the Kanban board than we spent adding issues to sprints with Scrum.
